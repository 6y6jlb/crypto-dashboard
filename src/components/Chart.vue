<template>
  <section v-if="this.selectedTicker" class="relative">
    <h3 class="text-lg leading-6 font-medium text-gray-900 my-8">
      {{ this.selectedTicker.Name }} - {{ this.currency }}
    </h3>
    <div
    ...
    >
      <div
        v-for="(item, idx) in this.chartValues"
        :key="idx"
    ...
    <button type="button" class="absolute top-0 right-0">
      <svg
        ...
        version="1.1"
        width="30"
        @click="this.$emit('toggle', this.selectedTicker.Id)"
        height="30"
        x="0"
        y="0"
...
export default {
  ...
  computed: {
    chartValues() {
      const values = this.values[this.selectedTicker.Name].reduce(
        (previous, current) => {
